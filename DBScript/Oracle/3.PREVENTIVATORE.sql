...
END;
 ...
 CREATE TABLE COSTIFISSIPREVENTIVO (  
    IDCOSTIFISSIPREVENTIVO NUMBER NOT NULL,  
...
END;
  ...
  CREATE TABLE ELEMENTICOSTIPREVENTIVI (  
    IDELEMENTOCOSTO NUMBER NOT NULL,   
    IDPREVENTIVOCOSTO NUMBER NOT NULL,
    IDELEMENTOPREVENTIVO NUMBER NOT NULL,
  RICARICO FLOAT NULL,
  COSTO FLOAT NULL,
  ...
  COSTOFIGLI FLOAT NULL,
  COSTOCOMPLETO FLOAT NULL,
  CANCELLATO VARCHAR2(1) NOT NULL,
  DATAMODIFICA DATE NOT NULL,
  UTENTEMODIFICA VARCHAR2(50) NOT NULL,
   CONSTRAINT PK_COSTOPREVENTIVO PRIMARY KEY (IDELEMENTOCOSTO),
   FOREIGN KEY (IDPREVENTIVOCOSTO) REFERENCES PREVENTIVICOSTI (IDPREVENTIVOCOSTO) ENABLE,
   FOREIGN KEY (IDELEMENTOPREVENTIVO) REFERENCES ELEMENTIPREVENTIVO (IDELEMENTOPREVENTIVO) ENABLE
   );
   
   CREATE INDEX IDX_PREVCOST_1 ON ELEMENTICOSTIPREVENTIVI (IDPREVENTIVOCOSTO);
   
   
  CREATE TABLE ELEMENTICOSTIPREVENTIVI_LOG (  
    IDELEMENTOCOSTO NUMBER  NULL,   
    IDPREVENTIVOCOSTO NUMBER  NULL,
    IDELEMENTOPREVENTIVO NUMBER  NULL,
  RICARICO FLOAT NULL,
  COSTO FLOAT NULL,
  ...
  UTENTEMODIFICA VARCHAR2(50)  NULL
   ...
   create or replace TRIGGER ELEMENTICOSTIPREVENTIVI_LOG
AFTER INSERT OR UPDATE ON ELEMENTICOSTIPREVENTIVI 
FOR EACH ROW
BEGIN
INSERT INTO ELEMENTICOSTIPREVENTIVI_LOG(IDELEMENTOCOSTO,IDPREVENTIVOCOSTO,IDELEMENTOPREVENTIVO,RICARICO,COSTO,INCLUDIPREVENTIVO,IDESTERNA,TABELLAESTERNA,PEZZIORARI,QUANTITA,COSTOARTICOLO,COSTOFIGLI,COSTOCOMPLETO,CANCELLATO,DATAMODIFICA,UTENTEMODIFICA)
VALUES
(:NEW.IDELEMENTOCOSTO,:NEW.IDPREVENTIVOCOSTO,:NEW.IDELEMENTOPREVENTIVO,:NEW.RICARICO,:NEW.COSTO,:NEW.INCLUDIPREVENTIVO,:NEW.IDESTERNA,:NEW.TABELLAESTERNA,:NEW.PEZZIORARI,:NEW.QUANTITA,:NEW.COSTOARTICOLO,:NEW.COSTOFIGLI,:NEW.COSTOCOMPLETO,:NEW.CANCELLATO,:NEW.DATAMODIFICA,:NEW.UTENTEMODIFICA);
END;
...
UPDATE PROCESSI SET STANDARD='N' WHERE IDARTICOLO<>-1;
 ...
 CREATE TABLE PREZZIMATERIALE (  
    IDPREZZOMATERIALE NUMBER GENERATED BY DEFAULT ON NULL AS IDENTITY,  
  ...
  DATAMODIFICA DATE NOT NULL,
  UTENTEMODIFICA VARCHAR2(50) NOT NULL,
   CONSTRAINT PK_PREZZOMATERIALE PRIMARY KEY (IDPREZZOMATERIALE),
    FOREIGN KEY (IDMATERIALE) REFERENCES MATERIALI (IDMATERIALE) ENABLE
   );


 CREATE TABLE PREZZIMATERIALE_LOG (  
   IDPREZZOMATERIALE NUMBER NULL,  
  ...
  UTENTEMODIFICA VARCHAR2(50)  NULL
   ...
   create or replace TRIGGER PREZZIMATERIALE_LOG
AFTER INSERT OR UPDATE ON PREZZIMATERIALE 
FOR EACH ROW
BEGIN
INSERT INTO PREZZIMATERIALE_LOG(IDPREZZOMATERIALE,DATAINIZIOVALIDITA,PREZZO,NOTA,IDMATERIALE,CANCELLATO,DATAMODIFICA,UTENTEMODIFICA)
VALUES
(:NEW.IDPREZZOMATERIALE,:NEW.DATAINIZIOVALIDITA,:NEW.PREZZO,:NEW.NOTA,:NEW.IDMATERIALE,:NEW.CANCELLATO,:NEW.DATAMODIFICA,:NEW.UTENTEMODIFICA);
END;
